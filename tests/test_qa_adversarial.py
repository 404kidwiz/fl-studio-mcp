...
class TestAdversarialInputs:
    ...
    @pytest.mark.parametrize("invalid_note", [
    ...
    @pytest.mark.parametrize("idx, length", [
        (-1, 8),
        (1000, 8),
        (0, -1),
        (0, 0),
    ...
    @pytest.mark.parametrize("idx, pan", [
        (-1, 64),
        (128, 64),
        (0, -1),
        (0, 128),
...
class TestAdversarialConcurrency:
    ...
    async def test_concurrent_query_serialization(self, dry_bridge):
        ...
        resp_status_payload = [1, 0, 120, 0, 2, 0, 4]  # playing, bpm_hi, bpm_lo, pat_idx, ch_count
        resp_channels_payload = [2, 4, 75, 105, 99, 107, 5, 83, 110, 97, 114, 101] # Kick (4), Snare (5)
        ...
        results = await asyncio.gather(task_status, task_channels, task_notes, return_exceptions=True)

        # 1. Verify no exceptions/errors occurred
        for res in results:
            assert not isinstance(res, Exception), f"Query raised an unexpected exception: {res}"
...
class TestAdversarialTimeouts:
    ...
    async def test_query_timeout_releases_lock(self, dry_bridge):
        ...
        t1 = asyncio.create_task(bridge.query(b"\xf0\x7d\x06\xf7", RESP_STATUS, timeout_ms=100))
        ...
        t2 = asyncio.create_task(bridge.query(b"\xf0\x7d\x06\xf7", RESP_STATUS, timeout_ms=100))
        res2 = await t2

        assert res2 == {"cmd": RESP_STATUS, "payload": resp_status_payload}, "Subsequent query should succeed after first one timed out"
