...
class TestWindowsAutomationFortification:
    ...
    def test_vbscript_timeout_graceful_recovery(self):
        ...
        with patch("tempfile.mkstemp") as mock_mkstemp, patch(
            ...
            mock_run.side_effect = subprocess.TimeoutExpired(
                cmd=["cscript", "//nologo", "hang.vbs"], timeout=5.0
            )
            mock_exists.return_value = True
    ...
    def test_vbscript_unhandled_exception_graceful_recovery(self):
        ...
        with patch("tempfile.mkstemp") as mock_mkstemp, patch(
            "os.fdopen"
        ) as mock_fdopen, patch("subprocess.run") as mock_run:
...
class TestWindowsMIDITransport:
    ...
    def test_midi_port_listing(self):
        ...
        with patch("mido.get_input_names") as mock_inputs, patch(
    ...
    def test_midi_port_opening(self):
        ...
        with patch("mido.open_input") as mock_open_in, patch(
            "mido.open_output"
        ) as mock_open_out:
